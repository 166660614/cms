...
class WxPayController extends Controller
{
    public $weixin_unifiedorder_url = 'https://api.mch.weixin.qq.com/pay/unifiedorder';
        public $weixin_notify_url = 'http://www.52self.cn/weixin/pay/notify';     //支付通知回调
        public function unifiedOrder($order_id){
           //请求参数
        ...
        public function notify(){
            ...
            if($xml['result_code']=='SUCCESS' && $xml['return_code']=='SUCCESS'){      //微信支付成功回调
                ...
                if($sign==$xml['sign']){       //签名验证成功
                    ...
                    $goodsData=GoodsModel::where(['goods_id'=>$orderData['goods_id']])->first();
                    GoodsModel::where(['goods_id'=>$orderData['goods_id']])->update(['goods_store'=>$goodsData['goods_store']-$orderData['order_amount']]);
                }else{
                    //TODO 验签失败
                    echo '验签失败，IP: '.$_SERVER['REMOTE_ADDR'];
                    // TODO 记录日志
                    file_put_contents('logs/wx_pay_ip.log',$_SERVER['REMOTE_ADDR'].'<br/>'.$sign,FILE_APPEND);
                }
